...
(function () {
  ...
  function wwHtml() {
    ...
    var items = hd && hd.holders && hd.holders.items ? hd.holders.items.slice(0, 100) : [];
    var rows;
    if (items.length) {
      ...
      rows = '<tr><td colspan="4"><span class="note" style="padding:12px 0;display:block">Couldn\u2019t load holders'
    ...
    return '<h1 class="page-title">' + esc(name) + ' <span style="font-variant:normal;color:var(--dim);font-size:20px">(' + esc(baseSym) + ')</span>'
      ...
      + '<div class="tbl-wrap"><div class="tbl-scroll holders-scroll"><table class="tbl"><thead><tr>'
  ...
  async function loadHolders() {
    ...
    function get(path) {
      return fetch(base + path)
        ...
        .catch(function (e) { holdersError = e.message || String(e); console.warn('[holders] ' + path + ' failed:', e); return null; });
    }

    // Fetch top-100 holders by following Blockscout's pagination cursor (50/page)
    async function holdersTop100() {
      var items = [], params = null, pages = 0;
      while (items.length < 100 && pages < 3) {
        var qs = '';
        if (params) {
          var sp = new URLSearchParams();
          Object.keys(params).forEach(function (k) { if (params[k] != null) sp.set(k, params[k]); });
          qs = '?' + sp.toString();
        }
        var page = await get('/api/v2/tokens/' + TOKEN_ADDRESS + '/holders' + qs);
