...
(function () {
  ...
  function wwHtml() {
    ...
    if (userBalRaw != null) {
      ...
      balBlock = '<div class="mybal"><span class="mcap-k">Your Balance</span>'
        + '<span class="mybal-usd" style="color:#b04a3a">balance unavailable \u2014 ' + esc(balanceError) + '</span></div>';
    } else {
      balBlock = '<div class="mybal"><span class="mcap-k">Your Balance</span>'
  ...
  async function loadBalance() {
    ...
    addr = m ? m[0] : null;
    var rpcs = RPC[tok.chain];
    if (!addr) { balanceError = 'no wallet address (raw: ' + (addr === null ? String(W && W.getAddress && W.getAddress()) : addr) + ')'; return null; }
    if (!rpcs) { balanceError = 'no RPC for chain "' + tok.chain + '"'; return null; }
    var data = '0x70a08231' + addr.slice(2).toLowerCase().padStart(64, '0');
    var body = JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'eth_call', params: [{ to: TOKEN_ADDRESS, data: data }, 'latest'] });
    var lastErr = '';
    ...
    balanceError = 'all RPCs failed (' + lastErr + ')';
    return null;
  }
