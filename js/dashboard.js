...
(function () {
  ...
  // Public RPCs for reading the connected wallet's token balance (keyless)
  var RPC = { base: 'https://base.llamarpc.com', ethereum: 'https://eth.llamarpc.com', arbitrum: 'https://arbitrum.llamarpc.com', optimism: 'https://optimism.llamarpc.com', polygon: 'https://polygon.llamarpc.com' };

  var tok = { chain: null, pair: null };
  var tokenPair = null, holderData = null, safeData = null, activePage = 'home';
  var refreshTimer = null, holdersError = null, userBalRaw = null;
  ...
  function wwHtml() {
    ...
    var balBlock = '';
    if (userBalRaw != null) {
      var balNum = Number(BigInt(userBalRaw)) / Math.pow(10, decs);
      var balStr = balNum.toLocaleString(undefined, { maximumFractionDigits: balNum >= 1 ? 2 : 6 }) + ' ' + baseSym;
      var price = pair ? Number(pair.priceUsd) : null;
      var usd = (price ? '\u2248 $' + (balNum * price).toLocaleString(undefined, { maximumFractionDigits: 2 }) : '');
      balBlock = '<div class="mybal"><span class="mcap-k">Your Balance</span>'
        + '<span class="mcap-v">' + balStr + '</span>'
        + (usd ? '<span class="mybal-usd">' + usd + '</span>' : '') + '</div>';
    } else {
      balBlock = '<div class="mybal"><span class="mcap-k">Your Balance</span>'
        + '<span class="mcap-v">\u2026</span></div>';
    }
  ...
  function securityHtml() {
    return '<h1 class="page-title">Security Module</h1>'
      ...
      + '<div class="tbl-wrap"><div class="note">More details coming soon.</div></div>';
  }
  ...
  async function loadBalance() {
    ...
    var rpc = RPC[tok.chain];
    if (!addr || !rpc) return null;
    var data = '0x70a08231' + addr.replace(/^0x/, '').toLowerCase().padStart(64, '0');
    var body = { jsonrpc: '2.0', id: 1, method: 'eth_call', params: [{ to: TOKEN_ADDRESS, data: data }, 'latest'] };
    var ctrl = new AbortController();
    var timer = setTimeout(function () { ctrl.abort(); }, 10000);
    try {
      var res = await fetch(rpc, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: ctrl.signal });
      clearTimeout(timer);
      var j = await res.json();
      if (j.error) throw new Error(j.error.message || 'rpc error');
      return (j.result && j.result !== '0x') ? j.result : '0x0';
    } catch (e) { clearTimeout(timer); console.warn('[balance] failed:', e); return null; }
  }
  ...
  function renderPage(key) {
    ...
    if (key === 'security') { c.innerHTML = securityHtml(); return; }
  ...
  async function ensureWwData() {
    ...
    loadBalance().then(function (b) { if (b != null) { userBalRaw = b; reWW(); } });
